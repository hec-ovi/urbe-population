...
export function staffWorkplace(seed: string | number, parcel: Parcel, floorArea: number): WorkplaceStaffing {
  ...
  if (profile.allDay) {
    const posts = Math.max(1, Math.round(capacity / 10));
    const securityPosts = Math.max(1, Math.round(posts / 2));
  ...
  return { slotCount: capacity, posts: 0, securityPosts: 0, allDay: false, nightOnly, openMin, closeMin };
}
